...
// ── Read side: company-scoped movement history ─────────────────────────────
...
/** Fetch movement history for a company, newest first, with variant/product labels. */
export async function getInventoryHistory(filter: InventoryHistoryFilter) {
  const take = Math.min(Math.max(filter.take ?? 50, 1), HISTORY_MAX_TAKE);
      ...
      select: {
        ...
        variant: {
          select: {
            ...
            size: true,
            color: true,
            product: { select: { id: true, articleNo: true, articleName: true } },
          },
        },
      },
    }),
